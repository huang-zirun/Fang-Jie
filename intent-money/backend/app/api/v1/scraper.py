...
async def search_douyin_videos(keyword: str, limit: int = 20):
    ...
    results = await douyin_scraper.search_hot_videos(keyword=keyword, limit=limit)
...
async def get_douyin_comments(video_id: str, limit: int = 50):
    ...
    comments = await douyin_scraper.get_video_comments(video_id=video_id, limit=limit)
    return {"video_id": video_id, "count": len(comments), "comments": comments}


@router.get("/health")
async def check_scraper_health():
    douyin_ok = await douyin_scraper.check_health()
    xhs_ok = await _xhs_scraper.check_health()
    return {
        "douyin": {"healthy": douyin_ok, "enabled": settings.SCRAPER_ENABLED and bool(settings.DOUYIN_COOKIE)},
        "xhs": {"healthy": xhs_ok, "enabled": settings.SCRAPER_ENABLED and bool(settings.XHS_COOKIE)},
    }
